...
public class Messaging implements MessagingInterface {
    ...
    private String imageMessagePath; // to be more implemented; the String that represents the image's path that wants to be sent
    ...
    public void sendMessage() throws InvalidException {
        ...
        ArrayList<String> msg = sender.getMessages();
        msg.add(messageContent);
        sender.setMessages(msg);
        System.out.println("Message sent: " + messageContent);
    }

    // A void method that deletes the message thrown in the parameter from the messages list and
    // throws an exception if the message passed in the parameter does not exist in the list
    public void deleteMessage(String targetMessage) throws InvalidException{
        if (sender.getMessages().contains(targetMessage)) {
            ArrayList<String> msg2 = sender.getMessages();
            msg2.remove(targetMessage);
            sender.setMessages(msg2);
    ...
    // An accessor method that returns the User who is sending the message
    public User getSender() {
        return sender;
    }

    // An accessor method that returns the User who is receiving the message
    public User getReceiver() {
        return receiver;
    }

    // An accessor method that returns the String message
